...
- **Phase 224** ✅ memory_agent Extractor-Robustheit – leere/Non-JSON-Antwort des Sonnet-Extractors (z.B. wenn „Vergiss nicht X" als Erinnerung statt Profil-Eintrag erkannt wird) führt nicht mehr zu `JSONDecodeError` + ERROR-Log, sondern zu sauberem `action=noop`; der Node antwortet ehrlich mit Erinnerungs-Hinweis statt der verwirrenden generischen „Möchtest du dass ich mir etwas merke"-Clarify-Frage; aufgedeckt beim E2E-Test von Phase 223; Issue #285; 1855 tests green
- **Phase 225** ✅ Routing für negierte Erinnerungen – neue deterministische `_pre_route_reminder`-Pipeline-Funktion routet „Vergiss nicht X" / „Nicht vergessen: X" / „Denk dran: X" zuverlässig zum `reminder_agent` (statt nicht-deterministischem LLM-Routing mal memory/mal reminder); Guard gegen Profil-Fakten (`dass ich/mir/mein` → an LLM, z.B. „Vergiss nicht dass ich Jazz mag"); `SUPERVISOR_PROMPT` geschärft (memory_agent-NEIN für negierte Erinnerungen + reminder_agent-Trigger); neues LLM-Routing-Golden-Set als skipif-Test (lokal ≥85% gegen echten Haiku, in CI geskippt); behebt nicht-deterministisches Routing aus E2E-Test Phase 223; Issue #286; 1868 tests green
