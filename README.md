...
- **Phase 158–167** ✅ Features & Bug-Fixes – system_agent via psutil (CPU/RAM/Disk), API Health-Check in Heartbeat (Anthropic/Tavily/Brave), Startup-Message on restart, Bug-Fixes #110–#114, Intent-Extraction via Haiku (Commitments in ChromaDB), Collector-Refactor (intent/person/place getrennt), Anthropic Prompt Caching (cache_control), Context-Injection-Fix (proactive messages in state), Supervisor Context Routing (last_agent_name), Weather Forecast by Day (wttr.in index); 1296 tests green
- **Phase 168–177** ✅ Security, CI & Routing – web.py hourly Bounds-Check, Photo follow-up Supervisor-Guard + vision_agent_name in State, Security Fixes (injection protection supervisor/memory/terminal), Bandit CI + weekly pip-audit, Multi-instance fcntl.flock + news freshness, _invoke_locks LRU-Eviction + Scheduler Liveness + Data Store Diagram, ruff CI + codebase formatting (91 files), Supervisor routing pipeline refactor (_PRE_ROUTE_PIPELINE), Self-Healing Watchdog Auto-Restart (launchctl), Memory-Reviewer Truncation-Bug fix (_is_valid_save superset-check); 1310 tests green
- **Phase 178–187** ✅ Robustheit, Proaktivität & Memory – Race-Condition-Fix + Frozen-Snapshot in write_profile() (deepcopy, optimistic-concurrency, atomic os.replace), Fork-Agent Learning Loop (Batch-Analyse alle N Turns), Node.js 24 Migration, Background Curator (wöchentliche Profil-Konsolidierung + Hotfix-Serie), Beziehungs-Alert (ChromaDB $lt-Filter, 14d/30d Schwellwerte), Memory-Parser zweistufig (Haiku-Router + Sonnet-Extractor), Curator Review-Fixes, Supervisor Routing Fix (#161), merke-dir-das Profil-Fakt vs. Bot-Instruktion (#164); 1452 tests green
