...
**Commands:**
```
/start /ask /clip /search /remember /briefing /done /done all /mute_proactive /tts on|off /stop
...
- **Maintenance 17.06.–28.07.2026** ✅ No phase work – four PRs keeping CI green. `pip-audit` and Dependabot findings in two rounds: `langsmith` 0.8.3→0.8.18 and `pydantic-settings` 2.13.1→2.14.2, both blocking *every* branch because the security-audit step runs before the test suite (#297); `pillow` →12.3.0 (13 advisories) and further pip/npm bumps (#300); `npm audit fix` in `whatsapp_service` for `qs`, `ws` (HIGH) and `js-yaml`, of which Dependabot had only surfaced `qs` — the other two were visible locally but never raised as alerts (#292). Plus a flaky test fixed at the root: `TestRunHeartbeat::test_sends_message_when_triggered` imported `is_focus_muted` locally, which read the **real** `~/.fabbot/activity.json` of the running bot, so the result depended on how recently the machine had been used (#298).
- **Phase 227** ✅ Conflict-Handler beendet Prozess statt Zombie – on Telegram `Conflict` the `_error_handler` now calls `os.kill(os.getpid(), signal.SIGTERM)` instead of `application.stop()`; SIGTERM triggers PTB graceful shutdown incl. `_post_shutdown` (WhatsApp/caffeinate cleanup) → launchd restarts cleanly (`ThrottleInterval=30` guards crash-loops). Previously `application.stop()` only halted polling while the process lived on as a zombie: schedulers kept firing (briefings went out) but incoming messages were dropped for days, and `KeepAlive` never recovered it (#295); 1882 tests green
- **Phase 228** ✅ youtube_agent Transcript-API 1.x Breaking Change – `youtube-transcript-api==1.2.4` dropped the static API (only instance-based `fetch`/`list` remain), so `_fetch_transcript` raised `AttributeError: type object 'YouTubeTranscriptApi' has no attribute 'get_transcript'` on **every** video and always fell through to the Whisper fallback: ~10 min of yt-dlp audio download, then failure on `no_speech_prob=0.66`. Fix: `YouTubeTranscriptApi().fetch(video_id, languages=[...])` + snippet objects (`.text`) instead of dicts, stale `# type: ignore[attr-defined]` removed (it had silenced mypy and hid the break). New `tests/test_ph228_youtube_transcript_api.py` tests `_fetch_transcript` for real instead of mocking it away as `test_ph190` did, incl. contract tests against the installed library that fail on the next upstream API change; verified live against the production video (6870 chars) (#301); 1893 tests green
- **Infrastructure block 09.–13.08.2026** ✅ Incident response, not phase work – eleven PRs, no phase numbers. **Crash cause (#308):** the venv lived on the external SSD; in standby the USB bus loses power, the volume disappears, and Python takes **SIGBUS** paging in its mmap'd `.so` files — 95 of 153 loaded libraries sat on `/Volumes/`. 18 Python processes died in one night, each wake correlating within 1–2 s; `KeepAlive` restarted the bot every time, which is why it was a chain rather than a single crash. venv moved to `~/.venvs/fabbot` on the internal disk, plist moved to `deploy/`. **WhatsApp chain:** a service orphaned by those crashes held port 8767 for ~10 h unnoticed, so `start_service()` now checks the port and identifies the owner before spawning (#310); Node's stdout went to `DEVNULL`, leaving no trace when the service hung 22 h on `ready:false`, now logged to `~/.fabbot/whatsapp_service.log` with start-time truncation above 5 MB (#311); with those logs the freeze became visible as total silence after `Authentifiziert` and was fixed by whatsapp-web.js 1.34.6→1.34.7, whose changelog names exactly *Frozen WhatsApp Start or Auth Timeout* — ready in 15 s instead of never (#314, #315); `stop_service()` only sent SIGTERM to the Node process, so puppeteer-Chrome children survived, held the session `SingletonLock` and blocked the next start at `authenticated` — now reusing `_terminate_orphan()` from #310 (parent + children SIGTERM, 5 s, then SIGKILL) (#317); new `GET /contacts` endpoint to read the real WhatsApp display names instead of guessing them for the `whatsapp_contacts` whitelist, verified against the live session with 433 contacts (#316). **Security:** `aiohttp` →3.14.3 and `cryptography` →50.0.0 against 6 CVEs that appeared after the last green run and blocked *every* PR, since the audit step aborts before the tests even start (#307); all 10 open Dependabot alerts closed, incl. `langgraph-checkpoint-sqlite` 3.0.3→3.1.1, which affects the main graph's checkpointer directly (#309); a GitHub PAT sat in cleartext in the git remote URL and was surfaced by every `git remote -v` — new `git_remote_guard.py` pre-commit hook blocks credentials in remotes (`always_run`, since `.git/config` is not versioned) and masks the credential in its own error output (#312). **Tooling:** `.graphifyignore` excludes `htmlcov/` and `graphify-out/`, which had made up 464k of 596k words in the graph corpus (#306); 1929 tests green
- **Phase 229** ✅ Conflict exit debounced – stops the self-feeding restart loop – Phase 227 killed the process on the **first** Telegram `Conflict`. But a Conflict is usually transient: after a restart Telegram keeps the `getUpdates` connection of the bot's *own* previous instance open for about a minute. Together with launchd `KeepAlive` that turned every blip into a loop (exit → launchd restarts → old connection still open → Conflict → exit → …), each iteration announcing "🔄 Bot gestartet."; observed on 12.08.2026 at 21:41:39 and 21:42:55, both ~70 s after start, with only **one** instance actually running (only PID 60549 held connections to `149.154.166.110`). Fix: new `bot/conflict_tracker.py` with `ConflictTracker.registriere()` counting conflicts in a sliding window (`STANDARD_SCHWELLE=3` within `STANDARD_FENSTER_SEKUNDEN=300`, injectable `zeitgeber` for tests); the tracker is instantiated per `build_bot()` call, not globally, and resets its window after firing. Below the threshold the handler only logs and polling continues. The Phase 227 anti-zombie guarantee is preserved — a real second poller keeps polling and trips the threshold within seconds — while isolated conflicts spread over hours no longer accumulate into a restart. New `tests/test_conflict_tracker.py` (7 tests: window boundaries, reset after firing, no accumulation, `schwelle=1` reproducing the old behaviour), `tests/test_ph227_conflict_exit.py` adapted to the threshold instead of removed (#313); 1921 tests green
- **Phase 230** ✅ Health Check without a reason – `❌ TTS: ` – the report of 14.08.2026 06:00 flagged TTS as failed but printed no cause, and the log held only `Health Check abgeschlossen – PROBLEME`. Two causes: (a) `httpx.ReadTimeout`/`ConnectTimeout` do **not** inherit from `asyncio.TimeoutError` and their `str()` is empty, so the `except asyncio.TimeoutError` branch in `_check_tts` was dead code for httpx failures and `return False, str(e)[:80]` yielded an empty string (check runtime 05:59:59,979 → 06:00:10,541 = 10.5 s at `_CHECK_TIMEOUT=10` confirms a timeout against `api.openai.com`); (b) `run_health_check` logged only OK/PROBLEME, so with the report going out via Telegram only, nothing was left to diagnose from. Fix: new `_fehlertext()` helper falling back to the exception class name so a reason is never empty, labelling `httpx.TimeoutException` explicitly as `Timeout (ReadTimeout)`, applied to all 12 check branches plus the `asyncio.gather` branch; failed checks are now logged with their detail at WARNING. New `tests/test_ph230_health_check_fehlertext.py` (16 tests incl. the inheritance assumption itself, so the timeout branch cannot regress) (#319); 1945 tests green
- **Phase 231** ✅ TTS survives the OpenAI hangs – the timeout Phase 230 made visible turned out to be external: **~15 % of all requests to `api.openai.com` hang outright** (measured 14.08.2026, 5 of 33; TCP connect succeeds in ~15 ms, then not a single byte arrives). Reproduced with curl *and* httpx outside the bot; ruled out: bot process, event loop (max 0.13 s blocked), API key (HTTP 200, 130 models), proxy, fd leak, IPv6 (no AAAA record), MTU (1472 B with DF-bit clean), a single bad Cloudflare IP (both target IPs equally affected) — and host-specific, with `cloudflare.com` and `api.anthropic.com` at 8/8 OK. Two consequences fixed: (a) `bot/tts.py:_synthesize_openai()` only retried on `{429, 503}`, so a hang fell straight through to the edge-tts fallback after the full 30 s client timeout — roughly every seventh voice output left the user waiting half a minute for the worse voice; now `_TTS_TIMEOUT=12.0` with `httpx.TimeoutException` in the retry path and no backoff (unlike 429, a hang is independent of the next attempt); (b) `_check_tts()` had no retry, colouring roughly every 6th morning report red for nothing — now 2 attempts à `_TTS_CHECK_TIMEOUT=5.0`, so the product stays at `_CHECK_TIMEOUT` and the report takes no longer than before. Live against the real API, 12 runs: 4 hit a hang, the retry caught 3 of them, 1 stayed red (was 4). New `tests/test_ph231_tts_timeout_retry.py` (11 tests, incl. 429 backoff preserved and non-timeout errors still falling through unretried) (#321); 1956 tests green
- **Phase 232** ✅ A single underscore swallowed the morning briefing – on 16.08.2026 no briefing arrived. It had been fully generated (13.5 s of LLM calls and RSS fetches) and was lost on send: `Can't parse entities: can't find end of the entity starting at byte offset 360`. Telegram's legacy Markdown rejects a message **entirely** when one `*`, `_` or backtick is unbalanced, and `bot/briefing.py` sent with `parse_mode="Markdown"` and no fallback. Reproduced by regenerating the briefing: the culprit was a pending item literally named `youtube_agent` — one underscore opening an italic entity that never closed, so it would have failed every morning until that item disappeared. The briefing text is almost entirely foreign input (RSS headlines, calendar titles, pending items, reminders in the user's own wording), so it cannot be reliably kept balanced, and escaping legacy Markdown is error-prone. Fix: new `bot/telegram_markdown.py` with `mit_markdown_fallback(senden, text, **kwargs)` — retries the same message without `parse_mode` when Telegram reports a parse error, logging a WARNING; other `BadRequest`s (blocked chat, unknown chat_id) and network errors pass through untouched, and a failing second attempt is not swallowed. Applied to all **12** call sites, since the same bug sat in the reminder scheduler (user text), the health report (exception strings), party report, curator report and the voice-transcription previews (`f"_{text}_"` breaks on an underscore in the spoken text). A structural test fails on any new unguarded `parse_mode="Markdown"` under `bot/`. Verified against the real error string from that morning (#326); 1965 tests green
- **Phase 233** ✅ Briefing keeps its formatting – the Phase 232 fallback saved the briefing from being lost, but then fired **every single morning**: the pending item `youtube_agent` kept breaking the Markdown, so for eleven days the briefing arrived unformatted — delivered 8/8 days, but with no bold headings and no structure. A fallback is a safety net, not a steady state. Cause: `generate_briefing()` interpolated four foreign-text sections into a Markdown template unguarded — weather (Open-Meteo), calendar (AppleScript event titles), pending items (Second Brain), news (RSS via Haiku). None of them produces Markdown deliberately; the formatting comes solely from the template, yet any `_` or `*` in them broke it. Fix: `telegram.helpers.escape_markdown(text, version=1)` applied to all four sections before interpolation — one place, covers every source, no hand-rolled escaping. Verified against live data: `youtube\_agent` escaped, all 12 remaining `*` are the template's own, balanced. New `tests/test_ph233_briefing_escaping.py` (10 tests incl. a balance check per section and proof the Phase 232 fallback is no longer triggered) (#329); 1976 tests green
- **Phase 234** ✅ Session summaries stop recycling old conversations – all four September sessions with content described topics from **15 August** ("delete all open items", "morning briefing didn't arrive", "Amalia not in the WhatsApp contact list") on days where the log shows zero to one incoming message. On top of that, **65 of 153** session files were pure placeholders ("Keine neuen Informationen"), their share climbing from 0 % in April to 77 % in August. Cause: `_get_messages_from_state()` returned the **entire** thread state — 5041 messages at the time of the fix — with no time filter and no record of what had already been summarised, so `_format_for_summary()` kept condensing the same trailing `_MESSAGE_WINDOW` messages every night. LangChain messages carry no timestamp (`additional_kwargs` is empty), so filtering by date is impossible. Fix: a high-water mark per chat in `~/.fabbot/session_summary_state.json` — only messages beyond it are summarised, and with nothing new **no file is written at all** instead of a placeholder. The mark advances only after a successful write, so a failed LLM call doesn't silently drop a day, and it is discarded when the state shrinks below it (checkpoint cleanup, reset). New `tests/test_ph234_session_hochwassermarke.py` (11 tests incl. proof that old messages no longer reach the prompt) (#333); 1990 tests green
- **Phase 235** ✅ `/done` finally sticks – a pending item marked done via `/done` reappeared in the next briefing, at the latest when the user asked the bot in plain language to remove it. The 09.09.2026 session notes recorded it verbatim: *"Nachricht 'Hallo' an Fabio (wieder aufgetaucht)"*. Phase 234 had blamed the recycled session summaries – only half the story. Cause: `collector.py` and `intent_extractor.py` wrote `"status": "open"` and `created_at=now` on **every** upsert, reading the existing record only for `mention_count`. Every further mention of a done item reopened it — including the deletion request itself, since every message runs through both extractors after the reply. Live DB confirmed it: `"Nachricht 'Hallo' an Fabio schicken"` open with `mention_count: 4`, two near-identical siblings done. Fix: shared `_existing_metadata()` / `_inherit_from_existing()` in `collector.py` carry `status`, `created_at` and `mention_count` over from the existing record; `last_mentioned_at` still updates. Second gap: there was no bulk deletion at all — `mark_done()` is only reachable via `/done <name>`, no agent has a tool for it, so "delete all open items" was confirmed by the bot and did nothing. New `/done all` marks every open item done in one batch `update`, behind a HITL confirmation via `bot/confirm.py`. Reproduced end-to-end against a temporary ChromaDB before and after. New `tests/test_ph235_done_bleibt_done.py` (25 tests) (#338); 2015 tests green
