...
- **Phase 178** ✅ Race-Condition-Fix + Frozen-Snapshot in write_profile() – load_profile() now returns deepcopy (prevents cache mutation); write_profile() gains optional expected_base_hash for optimistic-concurrency-control (STALE result on conflict); _write_profile_bytes() uses atomic os.replace(); frozen _profile_snapshot (TTL=300s, PROFILE_SNAPSHOT_TTL env) for stable Anthropic prefix-cache within a session; invalidate_chat_cache() removed from per-write path, only called on snapshot refresh; all 4 call-sites (memory_agent, profile_learner, whatsapp) migrated with STALE retry; 1332 tests green
