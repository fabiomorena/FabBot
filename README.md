...
- **Phase 222** ✅ HITL vollständig graph-nativ via LangGraph `interrupt()` – Meta-Issue #274: calendar_agent (#275), computer_agent (#276), whatsapp_agent (#277) von `__CONFIRM_*__`-Magic-Strings auf `interrupt()` migriert (analog Phase 212 terminal/file); `bot._handle_interrupt()` als einheitliche Bestätigungs-Brücke, Resume via `Command(resume=...)`, `decision` als Source of Truth gegen LLM-Drift bei Node-Re-Execution; whatsapp-Versand (async I/O) passiert im Node nach Resume; vision brauchte keine Migration (Marker längst tot); finaler Cleanup (#278) entfernt toten `CONFIRM_*`-Proto-Code (Konstanten + `is_confirm_*`/`is_any_confirm`), `_RESPONSE_DISPATCH` enthält nur noch `SCREENSHOT`; Nebenbei: Wetter-Briefing Brightsky-Fallback (#271), zwei CI-Test-Flakes gefixt (#273); Issues #242–245; 1834 tests green
