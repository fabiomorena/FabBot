...
- **Phase 297** ✅ Conflict-Exit entprellt – Neustart-Schleife nachts – Phase 295 called `os.kill(SIGTERM)` on the **first** Telegram `Conflict`. But a Conflict is often transient: after a network drop or the Mac's sleep/wake, Telegram briefly keeps the orphaned `getUpdates` connection registered. Combined with launchd `KeepAlive` every such blip became a full restart plus a "🔄 Bot gestartet." message – 16 restarts between 01:33 and 07:38 in one night. Fix: `_error_handler` now counts Conflicts in a sliding window (`_CONFLICT_EXIT_THRESHOLD=3` within `_CONFLICT_WINDOW_SECONDS=120`) and only exits once the threshold is reached; the counter lives per `build_bot()` call, not globally. A real second poller keeps polling and trips the threshold within seconds, so the Phase 295 anti-zombie guarantee is preserved, while isolated blips no longer restart the process. `tests/test_ph295_conflict_exit.py` extended accordingly (single conflict → no exit, repeated → SIGTERM, spread out beyond the window → no exit, per-app isolation); 1891 tests green
