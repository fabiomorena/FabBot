...
def _build_chat_prompt() -> str:
    """
    Baut den Chat-Prompt dynamisch – mit Cache (TTL 60s).

    Phase 95: Ergebnis wird gecacht um 3 Disk-Reads pro Message zu vermeiden.
    Cache wird via invalidate_chat_cache() sofort ungültig gesetzt wenn
    Profil, claude.md oder Session-Summaries sich ändern.
    ...
    # Cache-Hit: gültig und nicht abgelaufen → sofort zurückgeben
...
async def chat_agent(state: AgentState) -> AgentState:
    ...
    Phase 95: _build_chat_prompt() nutzt Cache (TTL 60s) – kein Disk-Read pro Message.
    Cache wird via invalidate_chat_cache() sofort zurückgesetzt wenn Profil /
    claude.md / Session-Summaries sich ändern.
    """
    llm = get_llm()
    ...
    # Phase 95: Prompt aus Cache (kein Disk-Read wenn Cache gültig)
