"""
Text-to-Speech fuer FabBot – Phase 69 Fix.
Aenderung: _get_openai_api_key() Helper damit Tests patchbar bleiben.
"""
import asyncio
import logging
import os
...
if OPENAI_TTS_VOICE not in _VALID_VOICES:
    logger.warning(f"Unbekannte OPENAI_TTS_VOICE: {OPENAI_TTS_VOICE!r} – erlaubte Werte: {sorted(_VALID_VOICES)}")
if OPENAI_TTS_MODEL not in _VALID_MODELS:
    logger.warning(f"Unbekanntes OPENAI_TTS_MODEL: {OPENAI_TTS_MODEL!r} – erlaubte Werte: {sorted(_VALID_MODELS)}")
...
def _get_openai_api_key() -> str:
    """
    Lazy read des OPENAI_API_KEY.
    Patchbar in Tests: patch('bot.tts._get_openai_api_key', return_value='sk-test')
    Lazy statt Modul-Global – konsistent mit llm.py, Key-Rotation ohne Neustart.
    """
...
async def _synthesize_openai(text: str) -> bytes | None:
    """Generiert Audio via OpenAI TTS API. 1 Retry bei 429/503."""
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            for attempt in range(2):
                resp = await client.post(
                    ...
                    json={"model": OPENAI_TTS_MODEL, "input": text, "voice": OPENAI_TTS_VOICE},
                )
