...
class TestIsDuplicate:
    def test_erste_verarbeitung_nicht_duplikat(self):
        from bot.bot import _is_duplicate
        update = _make_update(1001)
        assert _is_duplicate(update) is False

    def test_zweite_verarbeitung_ist_duplikat(self):
        from bot.bot import _is_duplicate
        update = _make_update(1002)
        assert _is_duplicate(update) is False
        assert _is_duplicate(update) is True

    def test_verschiedene_ids_kein_duplikat(self):
        from bot.bot import _is_duplicate
        assert _is_duplicate(_make_update(2001)) is False
        assert _is_duplicate(_make_update(2002)) is False
        assert _is_duplicate(_make_update(2003)) is False

    def test_keine_message_kein_crash(self):
        from bot.bot import _is_duplicate
        update = _make_update_no_message()
        assert _is_duplicate(update) is False

    def test_id_wird_in_deque_gespeichert(self):
        from bot.bot import _is_duplicate, _processed_message_ids
        _is_duplicate(_make_update(3001))
        assert 3001 in _processed_message_ids

    def test_duplikat_nicht_erneut_in_deque(self):
        from bot.bot import _is_duplicate, _processed_message_ids
        _is_duplicate(_make_update(4001))
        _is_duplicate(_make_update(4001))
        assert list(_processed_message_ids).count(4001) == 1


class TestDequeSemantics:
    def test_maxlen_ist_200(self):
        from bot.bot import _processed_message_ids
        assert _processed_message_ids.maxlen == 200

    def test_fifo_verdraengung(self):
        from bot.bot import _is_duplicate, _processed_message_ids
        # 200 IDs füllen
        for i in range(200):
            _is_duplicate(_make_update(i))
        assert len(_processed_message_ids) == 200
        assert 0 in _processed_message_ids

        # 201. ID verdrängt ID 0
        _is_duplicate(_make_update(200))
        assert len(_processed_message_ids) == 200
        assert 0 not in _processed_message_ids
        assert 200 in _processed_message_ids

    def test_verdraengte_id_nicht_mehr_duplikat(self):
        from bot.bot import _is_duplicate
        # 200 IDs füllen – ID 0 wird verdrängt
        for i in range(200):
            _is_duplicate(_make_update(i))
        _is_duplicate(_make_update(200))  # verdrängt ID 0

        # ID 0 kann jetzt wieder verarbeitet werden (nicht mehr Duplikat)
        assert _is_duplicate(_make_update(0)) is False

    def test_kein_unbegrenztes_wachstum(self):
        from bot.bot import _is_duplicate, _processed_message_ids
        for i in range(500):
            _is_duplicate(_make_update(i))
        assert len(_processed_message_ids) == 200
