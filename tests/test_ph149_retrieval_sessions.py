...
def test_index_file_skips_session_dir(tmp_path):
    ...
    with (
        patch("agent.retrieval._SESSIONS_DIR", tmp_path / "Sessions"),
        patch("agent.retrieval._get_collection", return_value=MagicMock()),
    ):
        from agent import retrieval

        result = asyncio.get_event_loop().run_until_complete(retrieval.index_file(session_file))
...
def test_index_file_indexes_wissen_file(tmp_path):
    ...
    with (
        patch("agent.retrieval._SESSIONS_DIR", tmp_path / "Wissen" / "Sessions"),
        ...
        result = asyncio.get_event_loop().run_until_complete(retrieval.index_file(note, force=True))
...
def test_index_all_calls_remove_sessions(tmp_path):
    ...
    with (
        patch("agent.retrieval._remove_claude_md_from_index", new=AsyncMock()),
...
def _stems(n: int, days_back_start: int = 0) -> list[str]:
    ...
    today = date.today()
    return [(today - timedelta(days=days_back_start + i)).strftime("%Y-%m-%d") for i in range(n)]
...
def test_load_sessions_fallback_to_last_when_all_old(tmp_path):
    ...
    _make_session_files(tmp_path, old)
