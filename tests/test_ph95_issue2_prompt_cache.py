...
def test_cache_hit_no_disk_read():
    ...
    with (
        patch("agent.claude_md.load_claude_md", fake_load_claude_md),
...
def test_cache_miss_after_ttl():
    ...
    with (
        patch("agent.claude_md.load_claude_md", fake_load),
        patch("bot.session_summary.load_session_summaries", return_value=""),
        patch("agent.profile.get_profile_context_full", return_value=""),
        patch.object(ca, "_PROMPT_CACHE_TTL", 0.05),
    ):  # 50ms TTL für Test
...
async def test_write_profile_invalidates_cache():
    ...
    with (
        patch("agent.profile._PROFILE_PATH") as mock_path,
        ...
        with patch("yaml.safe_load", return_value=mock_profile), patch("yaml.dump", return_value=serialized):
...
async def test_add_note_invalidates_cache():
    ...
    with (
        ...
        with patch("yaml.safe_load", return_value={"notes": []}), patch("yaml.dump", return_value="notes:\n- note\n"):
...
async def test_summarize_session_invalidates_cache():
    ...
    with (
        patch("bot.session_summary._get_messages_from_state", return_value=fake_messages),
